...
fn test_option_into_value() {
    ...
    assert_eq!(IntoValue::<i32>::into_value(opt_i), Value::Single(Scalar::I32(123)));

    // Option<String> (Owned) -> Value
    let opt_s: Option<String> = Some(String::from("inner"));
    assert_eq!(IntoValue::<String>::into_value(opt_s), Value::Single(Scalar::String("inner".into())));

    // Option<&str> -> Value
    let opt_str: Option<&str> = Some("inner_ref");
    assert_eq!(IntoValue::<String>::into_value(opt_str), Value::Single(Scalar::String("inner_ref".into())));

    // None 情况测试 (针对不同 T)
    let none_i: Option<i32> = None;
    assert_eq!(IntoValue::<i32>::into_value(none_i), Value::Single(Scalar::Null));

    let none_s: Option<String> = None;
    assert_eq!(IntoValue::<String>::into_value(none_s), Value::Single(Scalar::Null));
}
